# -*- coding: utf-8 -*-
"""
Created on Sun Sep 30 19:29:05 2018
...
if not use_gpu:
    with torch.no_grad():
        if use_gpu:
            ...
            h_t2 = Variable(torch.zeros(train_data_input.shape[0], hidden_featrues, dtype=torch.double))
            c_t2 = Variable(torch.zeros(train_data_input.shape[0], hidden_featrues, dtype=torch.double))
        power_lstm.load_state_dict(torch.load('training_weightsdaily.pt',map_location=lambda storage, loc: storage))
        fit_outputs = []
        for i in range(0,train_data_input.shape[1]):
...
#            fit_out,h_t,c_t, = power_lstm(train_data_input[:,i].reshape(1,-1),h_t,c_t)
        ...
        r2_lossontrain = r2_score(fit_outputs_transformed.reshape(fit_outputs_transformed.shape[0]),global_active_power_data[1:train_len+1])
...
if not use_gpu:     
    ...
    plt.plot(global_active_power_data[0:100].reshape(100),'b')
    plt.plot(fit_outputs_torch.detach().numpy()[0,0:100],'r')
    plt.gca().legend(('actual','prediction'))
#hours
"""MAE loss forecast = 0.1889
MAE loss pred = 0.22
R2 loss pred = 0.92
long_loss = 1.0479"""
